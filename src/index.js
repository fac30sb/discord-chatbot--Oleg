...
const { token, apiKey } = require('../config.json'); // Import Discord bot token and OpenAI API key from config file
                                                     ...
                                                     // Use a template file like config-example.json to guide users on creating their own config
...
async function makeChatGPTApiCall(question, channel) {
    try {
        ...
        // Construct messages array using conversation history
        const messages = conversationHistory.map(entry => ({
            role: 'user',
            content: entry.question,
        }));
        // Add system message as the first element in the messages array
        messages.unshift({
            role: 'system',
